...
const emit = defineEmits<{
  (event: 'sendLoading', val: boolean): void
}>()
const changeLoad = (isLoading: boolean): void => {
  emit('sendLoading', isLoading)
}
// 获取并存储猜你喜欢数据
...
const getHomeGoodsLikeData = async () => {
  ...
  // 推出判断
  if (finish.value === true) return uni.showToast({ icon: 'none', title: '没有更多数据了~' })
